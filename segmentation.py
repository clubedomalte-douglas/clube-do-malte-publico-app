...
def _normalize_phone_br(p):
    """
    Normaliza um telefone para o padrao exigido pela plataforma de WhatsApp:
    55 + DDD (2 digitos) + numero (9 digitos) = 13 digitos no total.

    Retorna a string normalizada ou None se nao for possivel normalizar com
    seguranca. Trata o caso comum em que o telefone vem como float do Excel
    (ex.: 21991842972.0), o que sem cuidado gera um '0' espurio ao final.
    """
    if p is None:
        return None
    if isinstance(p, float):
        if np.isnan(p):
            return None
        p = int(p)  # remove o ".0" sem deixar zero espurio
    digits = ''.join(ch for ch in str(p) if ch.isdigit())
    if not digits:
        return None

    # remove prefixo de codigo do pais (55) se ja vier incluso
    if digits.startswith('55') and len(digits) in (12, 13):
        digits = digits[2:]
    # remove prefixo de tronco (0) se vier incluso, ex.: 0DDD9XXXXXXXX
    if digits.startswith('0') and len(digits) in (11, 12):
        digits = digits[1:]

    if len(digits) == 10:
        # DDD + numero de 8 digitos (formato antigo) -> insere o 9 na frente do numero
        digits = digits[:2] + '9' + digits[2:]
    elif len(digits) != 11:
        return None  # nao da para normalizar com seguranca

    ddd = digits[:2]
    if not (11 <= int(ddd) <= 99):
        return None
    if digits[2] != '9':
        return None  # numero de celular deve ter 9 como primeiro digito apos o DDD

    return '55' + digits


def _valid_phone(p):
    return _normalize_phone_br(p) is not None
